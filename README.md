...
cd theme-slider
pnpm install
node_modules/.bin/vite build      # 构建 → assets/theme-slider.js|css
node_modules/.bin/vitest run      # 跑逻辑单测
```

> pnpm 11 默认拦 esbuild 构建脚本，`pnpm build` / `pnpm test` 会被 deps 预检打挂；先跑一次 `pnpm approve-builds` 批准 esbuild 后即可改用 `pnpm build`。上面直跑二进制无此问题。
