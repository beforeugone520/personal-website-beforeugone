...
Contribution `date` uses `YYYY-MM-DD`; `level` is GitHub's normalized contribution intensity mapped to `0` through `4`. GitHub aligns the rolling calendar to whole weeks, so a complete contiguous response contains 365 through 371 days rather than always exactly 365 or 366. In tokenless mode, `total_contributions` is computed from the parsed daily counts and accepted only when it matches the public page heading. `repositories` contains at most six public, owner-affiliated, non-fork, non-archived repositories ordered by most recent push; `archived` is retained in the stable response shape and is `false` for these active rows. Description/language fields can be empty. Repository strings originate from public GitHub data and must still be rendered with DOM text APIs. Consumers should use `refreshed_at` as the freshness signal and tolerate an older snapshot.

The process attempts a background refresh at startup and every `GITHUB_REFRESH_INTERVAL`, bounded by `GITHUB_REQUEST_TIMEOUT` for each upstream request. When `GITHUB_API_TOKEN` is configured, it fetches the complete dataset from the official GitHub GraphQL API. Without a token, it combines public repository metadata from the GitHub REST API with the user's public rolling contributions HTML; tokenless attempts, including webhook wakeups, are kept at least two minutes apart. The browser uses neither upstream path and calls only this backend route.

The rolling contributions HTML endpoint is an internal public GitHub web surface, not a versioned API contract. A markup change, parsing failure, incomplete response, or other upstream error therefore counts as a refresh failure. Regardless of source, only a structurally complete refresh atomically replaces the singleton `github_activity_cache` snapshot; failures never delete or partially overwrite the last-good snapshot. The public request reads SQLite only and does not synchronously proxy GitHub. GitHub refresh and HTML parsing failures do not affect `/healthz` or `/readyz`.

If `GITHUB_API_TOKEN` is configured, it must be public-read-only: classic `read:user`, `user`, and `repo` scopes, private-repository access, and fine-grained account permissions that expose non-public contributions are forbidden. The client rejects classic tokens whose response advertises one of those broad scopes.

The route sends `Cache-Control: public, max-age=300, stale-while-revalidate=3600`. It continues to return `200` while a last-good snapshot for the configured `GITHUB_USERNAME` exists, even when a later refresh failed. It returns `503 github_unavailable` with `Cache-Control: no-store` when the cache is empty or still belongs to a previously configured username. A configured `GITHUB_API_TOKEN` is server-only and is never included in the response. An accepted GitHub webhook may wake the background refresher, but webhook acceptance does not wait for or guarantee a refresh.
