...
> GitHub activity backend snapshot 与新版首页已于 2026-07-14 完成生产验收；其快照新鲜度现在属于日常巡检范围。
...
GitHub activity 不连接 OpenClaw：`beforeu-api` 后台配置可选 server-only token 时调用官方 GitHub GraphQL，未配置时组合 GitHub REST 公开仓库元数据与公开 rolling contributions HTML，将完整 last-good snapshot 写入 SQLite，再由 `GET /v1/public/github` 只读提供。它不是浏览器到 GitHub 的同步代理。
...
| 当前后端构建 | Git commit `a31f299` 的后端代码 |
| 当前二进制 SHA-256 | `1f951c5289e25e98975dae3d1ed67cba102c1a9afdd87dc18bc9d79cce7431a4` |
| 服务器源码环境 | 没有本项目 checkout，也没有 Go、`gh` 或 `jq`；不得在 Hermes 临时搭构建链 |
| 回滚基线 | `/usr/local/bin/beforeu-api.prev-20260714T033002Z-9815949`；回滚执行仍须先确认 |
...
回滚必须先确认。当前 Hermes 已保留版本化的 `9815949` 旧二进制；使用前仍要核对路径、checksum、migration 兼容性和对应数据库备份，不能仅凭文件名直接替换。
...
- `GITHUB_API_TOKEN`：当前留空；可选、server-only、公开读取最低权限，禁止 `read:user`/`user`/`repo` 及私有访问；真实值不得输出
...
2026-07-14 验收确认 `0002_github_activity_cache.sql` 已应用，DB/WAL 权限未改变，无 token fallback 的启动与 15 分钟周期刷新均能推进 `refreshed_at`。local/public route、cache header、last-good 保留、真实 push webhook 和前端响应式降级均已通过；详细时间、备份和浏览器证据见 `docs/backend-operations.md`。后续部署与配置修改仍必须先确认，并继续按本手册第 6 节执行二进制、checksum、备份和观察流程。
...
Hermes 没有 `gh`。创建、更新、删除、重送 webhook 必须先确认，并在 GitHub Settings 或已认证开发环境完成。Activity 的最小权限 `GITHUB_API_TOKEN` 只供 API 后台 refresh 使用，不得拿来运行 `gh`、管理仓库或扩大 OpenClaw 权限。
...
仓库中的人话 Ship 标题和 Now 最近动作仍是待发布候选，不属于当前生产基线。候选后端会在写入时按 Conventional Commit 类型生成稳定中文标题，静态前端会兼容已有旧式行；部署、受控 push、历史行展示、Now 14 天窗口和回退验收完成后，才能把这一段改成生产巡检要求并更新第 2 节 commit/checksum。
...
4. Now、Ship、guestbook、reactions 和 GitHub activity 的公开 GET 是否正常；检查 snapshot 状态、cache header 与 `refreshed_at`；公网 admin 是否仍是 `404`。
